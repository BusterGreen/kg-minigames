...
## Quiz
```lua
  local success = exports['kg-minigames']:Quiz({
...
## Word Guess
```lua
  local success = exports['kg-minigames']:WordGuess('fivem', 'the game modification you are playing on', 5) -- how many wrong guesses allowed
  if success then print('success') else print('fail') end
```

## Word Scramble
```lua
  local success = exports['kg-minigames']:WordScramble('fivem', 'the game modification you are playing on', 30) -- how long they have to unscramble in seconds
  if success then print('success') else print('fail') end
```

## Key Minigame
```lua
  local result = exports['kg-minigames']:KeyMinigame(10) -- amount of presses they need to do
...
## Lockpick
```lua
  local success = exports['kg-minigames']:Lockpick(5) -- number of tries
  if success then print('success') else print('fail') end
```
...
## Skillbar
```lua
  local success = exports['kg-minigames']:Skillbar() -- calling like this will use default easy difficulty with 1234
  if success then print('success') else print('fail') end

  local success = exports['kg-minigames']:Skillbar('medium') -- calling like this will just change difficulty and still use 1234
  if success then print('success') else print('fail') end

  local success = exports['kg-minigames']:Skillbar('easy', 'wasdfgh') -- calling like this will set difficulty and keys to press
  if success then print('success') else print('fail') end
```

## Pinpad
```lua
  local result = exports['kg-minigames']:StartPinpad(1234) -- numbers available to use are 1-9
